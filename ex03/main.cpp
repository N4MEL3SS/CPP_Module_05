...
int main()
{
	...
	// President:
	president_form = intern.makeForm("PresidentialPardonForm", "president");
	std::cout << *president_form << " was just created " << std::endl << std::endl;

	// Robo:
	robo_form = intern.makeForm("RobotomyRequestForm", "robo");
	std::cout << *robo_form << " was just created " << std::endl << std::endl;

	// Shrub:
	shrub_form = intern.makeForm("ShrubberyCreationForm", "shrub");
